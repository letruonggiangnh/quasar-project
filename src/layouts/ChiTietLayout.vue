  ...
  <q-layout view="lHh Lpr lFf">
    ...
    <div class="row">
      <h5 class="text-blue text-h5 q-mb-xs q-pt-md q-pl-md">
        ...
        Danh Sách Trạm <span class="q-pa-sm text-black">#</span>Chi Tiết
      </h5>
    </div>
...
const matram = router.currentRoute._rawValue.params.matram;
const provinces = [
  "Hà Nội",
  "Hồ Chí Minh",
  "Hải Phòng",
...
let config = {
  ...
  url: "http://localhost:5000/api/Tram/gettrambymatram",
  headers: {
    maTram: matram,
  },
};
...
axios
  .request(config)
  .then((response) => {
    const tram = response.data;
    kinhdo.value = tram.kinhDo;
    vido.value = tram.viDo;
    tentram.value = tram.tenTram;
    tinhModel.value = tram.tenTinh;
    loaiTramModel.value = tram.loaiTram;
    diachi.value = tram.diaChi;
    khuvucModel.value = tram.khuVuc;
    phanCapTramModel.value = tram.phanCapTram;
  })
  .catch((error) => {
    console.log(error);
  });

let configGetProvince = {
  method: "GET",
  maxBodyLength: Infinity,
...
axios
  .request(configGetProvince)
  .then((response) => {
    response.data.results.map((province) => {
      stringOptions.push(province.province_name);
    });
  })
  .catch((error) => {
    console.log(error);
...
const submitForm = () => {
  ...
  let config = {
    method: "post",
    maxBodyLength: Infinity,
    url: "http://localhost:5000/api/Tram/updatetram",
    headers: {
  ...
  axios
    ...
    .then((response) => {
      if (response.status == 200) {
        $q.notify("Cập nhật thành công");
      } else {
        $q.notify("Có lỗi xảy ra");
      }
    })
    .catch((error) => {
      console.log(error);
